...
それを使える人には誰にでも無料で差し上げます。時間とお金、プログラムと機材での貢献が大いに必要です。1
//}

経験を積んだ Unix の開発者にとって、このメッセージは理想主義と自信過剰を混ぜ合わせたものだった。著者はすでに成熟した Unix オペレーティング・システムを土台から再構築すると誓い、そのうえ、あちこちを改良することまで提案している。著者が予測する新しい GNU システムは、テキストエディタや Unix 互換アプリケーションを走らせるシェルプログラム、コンパイラ、「そしてその他いくつか」の一般的コンポーネントすべてを持っている。2 さらに、プログラミング言語 Lisp をベースにしたグラフィック・ユーザー・インターフェイスやクラッシュ耐性化されたファイルシステム、それに MIT のインターナル・ネットワーキング・システムに基づいたネットワーキング・プロトコルのような、まだ Unix システムが提供していない魅力的な機能がたくさん含まれるという。
...
//quote{
私はリチャード・ストールマンです。たくさん模倣されたオリジナル EMACS エディタの考案者で、今は MIT の人工知能研究所にいます。コンパイラ、エディタ、デバッガ、コマンドインタプリタ、非互換時分割システム（ITS）や Lisp マシン・オペレーティング・システムについて広く取り組んできました。ITS 端末の独立ディスプレイのサポートを開拓し、Lisp マシンにクラッシュ耐性ファイルシステムを一つ、ウィンドウシステムを二つ実装しました。3
//}
...
この移植性を円滑にするために、Unix の設計に従事していた開発者たちは、ソフトウェアとマシンの間に必ずもう一つ追加的抽象レイヤーを置くようにしていた。オペレーティングシステムを、AI ラボのハッカーが ITS や PDP-10 でしたように、特定のマシンのタイプの命令で書かないで、Unix の開発者は、C と呼ばれる高級言語で書いた。インターフェイスと仕様書が噛み合うことに焦点を合わせて、彼らはどんなマシンでも走るようにすぐに修正できるシステムを書いた。実際のコンポーネント自体よりも、むしろオペレーティング・システムの多くのサブコンポーネントをまとめるものに焦点を合わせた。もし、ユーザーがあるコンポーネントを気に入らなければ、インターフェイスの仕様書は、個別のサブコンポーネントを抜き出し、それを修繕し、あるいは置き換え、もっと良い何かにすることができた。簡単に言うと、Unix のアプローチは、柔軟性と経済性を促進するものだったため、急速に導入されていった。4
...
専念してくれるハッカーがほとんどいなければ、プログラムやマシンの修復にはもっと時間がかかる、つまり、それはちっとも修復されないということだった。ストールマンによれば、さらに悪いことにラボの「人口構造の変化」が起きた。かつて AI ラボで発言力のある少数派だったハッカーはその地位を失い、その一方で、「（PDP-10を）心から愛していない教授や学生が今までにないくらいの多数派になってしまった。」5
...
「『システムを管理するハッカーがいなければ大惨事になる。商用ソフトウェアを使うべきだ』と（教授陣は）言うのです。」ストールマンは数年後にこう振り返っている。「『会社がシステムを保守してくれることを期待できる』と言っていました。それで彼らは完全に間違っていると分かったけど、商用ソフトウェアを使うことになりました。」6

最初、ハッカーたちは Twenex システムも打倒されたがっている、もう一つの権威のシンボルと見ていた。システムの名前自体が異議の申し立てだった。公式には DEC が TOPS-20 と名付けていたが、それは DEC が PDP-10 用に販売していた商用オペレーティングシステム、TOPS-10 の後継だった。ボルト・べラネク・ニューマンは TOPS-20 の元になったその改良版を開発して、それを Tenex と名付けていた。7 Twenex という呼び名を考案したハッカーのストールマンは、TOPS-20 という名前を使わないで済むようにその呼び名を思いついたのだという。「システムはトップと呼ぶにはほど遠かったから、そんな名前で呼ぶしかなかったんです」とストールマンは回想している。「それで、Tenex という名前に『ｗ』を一つくっつけて、Twenex と呼ぶことにしました。」

Twenex/TOPS-20 システムを動かしているマシンにも、冷笑的な意味の『オズ』というあだ名があった。ハッカーの伝説の一つによれば、マシンにそのあだ名が付いたのは、端末に電力を供給するのにより小さい PDP-11 が一台必要だったからだ。あるハッカーが、KL-10 と PDP-11 のセットアップを初めて見たときに、それを映画『オズの魔法使い』の中の大げさな魔法使いの登場場面になぞらえた。「我こそは偉くて強いオズ」、そのハッカーは歌った。「コンソールのうしろの PDP-11 を気にすることはない。」8
...
パスワードを入手し、起動中にデバッガーを適用し、ストールマンはこれらの企てをうまく失敗させていた。二度目の「クーデター」を失敗させた後、ストールマンは AI ラボの全員に対して警報を発した。9
...
ITS 時代には MIT の管理者側も大目に見ていたこのいわゆる「観光客」ポリシーは、10  研究所の ARPAnet へのプライマリ・リンクがオズになってからは失われた。当初はストールマンも仲間が後から同じ手順で入れるようにログイン ID とパスワードを同じにするポリシーを続けていた。しかし、オズの脆弱さのため、時間が経つにつれ、全くの偶然あるいは悪意をもってシステムをダウンさせるかもしれない部外者たちを閉め出すように管理者たちは動いた。この管理者たちから自分のパスワードを公開することを最終的に止めるよう求められると、ストールマンは自分の倫理観を盾にしてそれを拒み、オズを使うことも一緒に止めてしまった。11

「MIT の AI ラボに初めてパスワードが登場した（ときに）、僕は自分の信念に従ってパスワードなしにする（ことを決めました）。」後に、ストールマンは語るだろう。「なぜなら、コンピュータにセキュリティを施すのが本当に望ましいことだとは信じられませんでした。ぼくがセキュリティ体制を喜んで手助けする筋合いではなかったのです。」12
...
シンボリクス社は、外部からの投資資金を使って多方面の AI ラボハッカーを募り、AI ラボの保護の外にある Lisp マシン・オペレーティングシステムのパーツを改良する仕事に配置した。1980 年の終わりまでに、Lisp マシンの自社ヴァージョンを開発するために 14 人の AI ラボ職員をパートタイム・コンサルタントとして雇った。残る少数は、ストールマンを別とすれば、LMI 社のために働いた。13 ストールマンは、プレッシャーのない AI ラボの生活を選び、どちらかの側に立つことを望まず、どちらの会社にも参加しないことに決めた。
...
//quote{
ストールマンが書いたものを見て、それは駄目だと決めつけることができたかもしれません（おそらく違うが、誰かがそれは駄目だと信じ込ませることができたかもしれない）。そうだとしても、「しかし、ちょっと待った。あちらじゃ、ストールマンは一晩中誰とも議論していないんだ。一人で働いているんだぜ！これを一人でやれるなんてことは信じられない！」と言ったでしょう。16
//}
...
「見返りなしでプロフェッショナルな仕事をする余裕が誰にあるだろうか。」とゲイツは問いかける。「プログラミングに三人年かけ、すべてのバグを発見し、製品を文書化して、無料で配布できるのは、どんなホビイストだろうか。」17
...
//quote{
もしも、私が私自身のためにいるのでなければ、誰が私のためにいるのだろうか。もしも、私が私自身のためにのみいるのだというなら、私は何なのだろうか。もしも、今でなければ、いつだというか。12
//}
...
「彼は馬鹿にした調子で『大学は自由です。しかし、コンパイラは自由じゃない。』と返事をよこしました。」とストールマンは回想する。彼は助けるのを断ったばかりか、GNU を開発する計画から手を引いて、そのかわりに VUCK の販売の強化になるアドオンでも書いたらどうかとストールマンに提案した。見返りに利益を分けるという。「だからこそ、GNU プロジェクトのためのぼくの最初のプログラムは、マルチ言語、マルチプラットフォームのコンパイラにしようと決めました。」19
...
「時々、思うことがあるんです。ひょっとして、自分の人生でできた最善のことは企業秘密になっている占有的ソフトウェアを大量に見つけて、そのコピーの街頭配布を開始することなんじゃないだろうか。そうすれば、企業秘密なんかじゃなくなってしまう。」とストールマンは言った。「ひょっとして、実際に自分で書いた新しい自由ソフトウェアを人にあげるよりも、自分にとってもっと効率的なんじゃないだろうか。しかし、怖いからそれは誰も受け取ることさえできません。」20
...
「もしも報酬を受けるに値するものが何かあるとすれば、それは社会的な貢献である。」ストールマンは書いた。「創造性は社会的貢献でありうる。しかし、それは社会がその結果を自由に使える限りにおいてのことだ。もしもプログラマが技術革新的プログラムの創造により報酬を受けるに値するとすれば、同じ理由により、もしもこれらのプログラムの使用を制限するならば、罰せられるに値するのである。」21
...
1『最初の GNU 声明』（1983年9月）リチャード・ストールマン、参照。

2 同書。

3 同書。

4『バークレー Unix の 20 年』マーシャル・カーク・マクージック、「オープンソース」（株式会社オライリー・ジャパン、1999年）を参照。

5 リチャード・ストールマン（1986年）参照。

6 同書。

7 複数の出典を参照。リチャード・ストールマンのインタビュー、ジェラルド・サスマンの電子メール、ジャーゴン・ファイル 3.0.0
...
10『MIT AI ラボの観光客ポリシー』を参照。

http://www.art.net/ hopkins/Don/text/ ~ tourist-policy.html

11 リチャード・ストールマン（1986年）参照。

12 同書。

13  Steven Levy, Hackers (Penguin USA [paperback], 1984): 423.
『ハッカーズ』スティーブン・レビー（工学社）

14  H. P. ニューキストの『頭脳制作者たち』が、AI ラボはストールマンに Lisp マシンプロジェクトから離れているように告げた、と言うのは誤りである。

15『ハッカーズ』で、スティーブン・レヴィがこの時期を念頭に記述したときに、ストールマンを「最後の真のハッカー」としていたが、彼の意図した意味はあなたが思ったものと違ったかもしれない。レヴィは「真のハッカー」という用語を MIT のハッカー・コミュニティを他の二つのハッカー・コミュニティから区別するために使い、後者は本の中で後で解説して、別の名前を与えた。[「真のハッカー」は第一部のタイトルでMITハッカーを指している。第二部はハードウェア・ハッカー、第三部はゲームハッカー。（訳者）]ストールマンだけを残して、このコミュニティが溶解したとき、彼は、結果として、最後の「真のハッカー」になった。レヴィは、他の者は誰も本当のハッカーではないと言おうとしているのではない。しかし、人々は、特にレヴィの本の説明を読んでいない人々は、彼の言葉をそのように解釈する傾向がある。ストールマン自身は、レヴィの本のそれらの言葉を使って自らを形容したことはない。

16 Steven Levy, Hackers (Penguin USA [paperback], 1984): 426 日本語訳は、『ハッカーズ』スティーブン・レビー（工学社）

17『ホビイストへの公開状』ビル・ゲイツ（1976年2月3日）。この公開状のオンライン上のコピーは次のサイトで見ることができる。

http://en.wikipedia.org/wiki/Open_Letter_ to_Hobbyists.

18 次を参照。ストールマンはこの声明に脚注を加えて、「無神論者の一人として、宗教指導者に従うことはないが、彼らの中の或る者が述べた言葉に感心している自分に気付くことがある。」と書いている。

http://www.gnu.org/gnu/the-gnu-project.html.

19 Richard Stallman, "The GNU Operating System and the Free Software Movement," Open Sources (O'Reilly & Associates, Inc., 1999): 65.を参照。日本語訳は、「GNU システムとフリーソフトウェア運動」、リチャード・ストールマン、『オープンソース』（株式会社オライリー・ジャパン、1999 年）。

20 リチャード・ストールマン（1986 年）

21『GNU宣言』（1985 年）リチャード・ストールマン
