...
改良する自由と著作者特権との間の緊張関係は GOSMACS よりも前にできていた。1976 年の著作権法は、アメリカの著作権法を見直し、著作権の法的保護をソフトウェアプログラムにまで拡大した。その法律のセクション 102（b）によれば、今や、個人と企業は「プログラムに具体化した現実のプロセスまたは方法」に対してではなく、ソフトウェアプログラムの「表現」に対して著作権を持てるようになった。1
...
//quote{
たぶん、ライセンスから「EMACS」を削除し、「ソフトウェア」とか何か別の名前で置き換えるべきです。遠からず、と希望しますが、Emacs は GNU システムの一番大きな部分ではなくなり、このライセンスが GNU システム全体に適用されることになるでしょうから。3
//}
...
またはそれを変更するとき、あなたは一定の責任を負うことになります。4
//}
...
ソフトウェアを買い占める人達がつくった法体系を、それに対抗するために使うんですから。」5
...
そのプログラムを修正・拡張した版も同じように自由ソフトウェアにすることを要求する」著作権ライセンスの手法全般を表現する自由ソフトウェア財団の専門用語になった。6
...
「ぼくは、ある程度まで再実装から利益を得ています。再実装すると大抵のシステムはずっと良くなるんです。それは、ある程度まで、ぼくがこの分野に長いこといて、他の多くのシステムでも仕事をしてきたからだと思います。だから、ぼくは沢山のアイデアを持ち込めるんです。」8
...
当時、すぐそばにあるヘルシンキ大学の学生だったトーバルズは、ストールマンをぼんやり眺めていた。2001 年の自伝『それがぼくには楽しかったから』の中で、トーバルズは、「ぼくは初めて、型どおりに長髪で髭をはやしたハッカーの典型を見た。ヘルシンキにはあまりいない人だった。」と回想している。13
...
トーバルズは Minix（ミニックス）を使っていたが、それはオランダの大学教授、アンドリュー・タネンバウム（Andrew Tanenbaum）が教育的手本として開発した自由のない軽量システムだった。14 それは自由のない自由大学コンパイラ・キットに加えて、タネンバウムが 1983 年の手紙でストールマンを軽蔑的に招待した類いのユーティリティを含んでいた。15
...
1991 年初頭に、トーバルズはターミナル・エミュレーションのプログラムを書き始めた。彼は自分のエミュレータの開発に Minix を使ったが、エミュレータは Minix では走らなかった。それは、スタンドアローン・プログラムだった。次に、彼はそれに Minix のファイルシステムにあるディスク・ファイルにアクセスする機能を与えた。その頃、トーバルズは彼の進化していく作品を「GNU/Emacs のターミナル・エミュレーション・プログラム」と呼んでいた。16

Minix が多くの重要な機能を欠いていたので、トーバルズは、彼のターミナル・エミュレータをMinix のカーネルにカーネル互換性を持つものへと拡張し始めた。ただ、モノリシックにした点は別だった。彼は意欲的に、Minix のニュースグループに、Unix 互換カ−ネルの標準仕様書、POSIX（ポジックス）規格のコピーを求めた。17 数週間後、カーネルと GNU プログラムの幾つかをまとめ、それらが一緒に動作するように調整し、トーバルズはメッセージをポストした。それは 1983 年のストールマンの最初の GNU のポストを連想させる。
...
（とくに（ごく実際的な理由から）ファイルシステムの物理的なレイアウトは同じです）。bash (1.08)と gcc (1.40)を現在移植中です。. . . 18
//}
...
当初、Linux は自由ソフトウェアではなかった。そのライセンスは自由の要件を満たさなかった。商業的配布を認めていなかったからである。トーバルズは企業が急襲して彼から Linux を取り上げてしまうんじゃないかと心配していた。しかし、トーバルズは GNU/Linux の組み合わせが人気を獲得するにつれ、コピーの販売はコミュニティに有益だと考えるようになった。そして、乗っ取りに対する懸念は心配するまでもないと感じ始めた。19 そこで、Linux のライセンスを考え直すことにした。

Linux を GCC でコンパイルしても、Linux で GCC を走らせても、GNU GPL で Linux をリリースする法的必要性はなかったが、トーバルズが GCC を使っていることは、他のユーザーにそれを借用させる道徳的責任を含意していた。トーバルズが後に述べたように、「ぼくは巨人の肩の上に乗っていた」のだった。20 彼が、他の人が同じような支援のために彼をあてにしてきたときに何が起きるか考え始めたのは驚くに当たらない。
...
誰かが改良を加えたら、それを自分のものにできるようにしたかったんです。21
//}
...
リナックスのプロジェクトが初期段階で下した「最良の設計上の意志決定の一つ」が GPL だったと彼は考えていた。22
...
1991 年のストールマンの科学技術大学での講演と、それに続く GPL への切り替えという決定を回想してトーバルズは次のように書いている。「ちゃんと理解してはいなかったかもしれないが、少なくとも彼の話の一部は頭の中に染み込んだんだと思う。」23

後注

1 Hal Abelson, Mike Fischer, and Joanne Costello, "Software and Copyright Law," 改訂版 (1998) を参照。

http://groups.csail.mit.edu/mac/classes/ 6.805/articles/int-prop/software-copyright.html

2. trn キットの README を参照。

http://stuff.mit.edu/afs/sipb/project/trn/src/ trn-3.6/README

3. ジョン・ギルモアの著者への e メールから引用。

4 リチャード・ストールマン他、『GNU一般公衆ライセンス 第１版』(1989年2月) を参照。

http://www.gnu.org/licenses/old-licenses/gpl-1.0.html

5 デイビット・ベッツ、ジョン・エドワード「リチャード・ストールマンがバイト誌編集者と彼のパブリック・ドメインの[原文のまま] Unix 互換ソフトウェアシステムについて語る」BYTE 誌 (1996 年 7 月) 参照。

(GNU プロジェクトのウェブサイトに再録。
http://www.gnu.org/gnu/byte-interview.html)

このインタビューは、GNU プロジェクトを始めた頃のストールマンの政治姿勢について、
率直さはもちろんだが、興味深いものを垣間見せてくれる。
ストールマンの言い回しの変化を跡付けるためにも有益だ。
...
2000 年 8 月の著者への次の声明とこれを対比されたい。
「あなたの思考の中で『知的財産』という用語を使わないように説得したい。それは事態を誤解するように導くでしょう。
なぜなら、その用語は、著作権、特許権、商標権を一緒くたにしているからです。
これらの効果は非常に異なっています。
それらを同時に語ろうとするのは全く馬鹿げています。
もし誰かが知的財産について、カッコ付きじゃなくて何か言っているときは、
その人は不明瞭に考えているから唱和すべきじゃないのです。」

[ RMS:  その対比が示すものは、ぼくが一般化に対して、より注意深くなることを学んだということだ。今日では、おそらくぼくは「知識の私物化」について語らないだろう、なぜなら、それは非常に広い観念だからだ。しかし「知識の私物化」は「知的財産」と同じような一般化ではない。これらの三つの法律の間の違いは、知識の私物化についてのいかなる法的問題の理解にも、決定的だ。特許権は一定の知識を使うことへの直接的独占だ。それは実際に「知識の私物化」の一形態だ。著作権は知識を体現する、または解説する作品の共有を止めるために使われる手法の一つで、全く違った事柄だ。他方、商標は知識という主題とはほとんど関係がない。]

6 カリフォルニア大学の「うざったい広告条項」は、その後、
問題があることが明らかになった。
ハッカーの中には、GPL よりも穏やかな制限を求めて、
カリフォルニア大学のライセンスを「カリフォルニア大学」というところだけ自分の組織の名前に置き換えて使っていた者もいた。
その結果、他のプログラムから数十ヶ所借りているプログラムは、
数十ヶ所の組織名を広告で引用する必要があった。
1999 年、ストールマン側の十年がかりの働きかけを経て、
カリフォルニア大学はこの条項を削ることに同意した。

BSD ライセンスが抱える問題参照。http://www.gnu.org/philosophy/bsd.ja.html

7 マイケル・ティーマン、「シグナスソリューションズ社の将来性」『オープンソース』（1999年、オライリー・ジャパン）143 ページを参照。

8 リチャード・ストールマン、BYTE誌 (1986 年) 参照。

9『HURD の歴史』を参照。

http://www.gnu.org/software/hurd/history.html

10 プログラミングの自由同盟プレスリリースによれば、
この抗議行動は、十六進プロテスト数え唄が始めて歌われたことで著名である。

//quote{
1-2-3-4、弁護士はドアから外に放り出せ@<br>{}
5-6-7-8、発明しろ（innovate）、訴訟（litigate）をするな@<br>{}
9-A-B-C、1-2-3 はいらないぞ@<br>{}
D-E-F-O、ルックアンドフィールはもうお仕舞い
//}

http://progfree.org/Links/prep.ai.mit.edu/demo.final.release

11 Reuven Lerner, "Stallman wins $240,000 MacArthur? award," MIT, The Tech (July 18, 1990). http://the-tech.mit.edu/V110/N30/rms.30n.html
を参照。

12 マイケル・グロス、「リチャード・ストールマン。高校の不適応者、自由ソフトウェアのシンボル、マッカーサー奨学金の天才」(1999 年) を参照。

13 リーナス・トーバルズ、デイビッド・ダイアモンド共著「それがぼくには楽しかったから」（小学館プロダクション、2001 年）99 ページ参照。この本はおそらく、トーバルズの人生に関しては正確だが、ストールマンについてはしばしば誤ったことを言っている、たとえば、ストールマンは「すべてをオープンソースにしたがっている、」そして「他の人が GPL を使わないことに不平を言っている。」と言う。実際には、ストールマンが擁護しているのは自由ソフトウェアで、オープンソースではない。彼は、ほとんどの環境で、作者に GNU GPL を選ぶように勧めるが、全ての自由ソフトウェアを倫理的だと言っている。
...
22 ロバート・ヤング「リナックス作成者リーナスとのインタビュー」リナックス・ジャーナル（1994 年 3 月 1 日号）を参照。
